...
public final class FibonacciHeap<T> {
	/*
	 * In order for all of the Fibonacci heap operations to complete in O(1), clients need to have O(1) access to any element in the
	 * heap. We make this work by having each insertion operation produce a handle to the node in the tree. In actuality, this
	 * handle is the node itself, but we guard against external modification by marking the internal fields private.
	 */
	...
	/**
	 * Inserts the specified element into the Fibonacci heap with the specified priority. Its priority must be a valid double, so
	 * you cannot set the priority to NaN.
	 *
	...
	/**
	 * Returns an Entry object corresponding to the minimum element of the Fibonacci heap, throwing a NoSuchElementException if the
	 * heap is empty.
	 *
	...
	/**
	 * Given two Fibonacci heaps, returns a new Fibonacci heap that contains all of the elements of the two heaps. Each of the input
	 * heaps is destructively modified by having all its elements removed. You can continue to use those heaps, but be aware that
	 * they will be empty after this call completes.
	 *
	...
	public static <T> FibonacciHeap<T> merge(FibonacciHeap<T> one, FibonacciHeap<T> two) {
		...
		/*
		 * Merge the two Fibonacci heap root lists together. This helper function also computes the min of the two lists, so we can
		 * store the result in the mMin field of the new heap.
		 */
	...
	public Entry<T> dequeueMin() {
		...
		/*
		 * Now, we need to get rid of this element from the list of roots. There are two cases to consider. First, if this is the only
		 * element in the list of roots, we set the list of roots to be null by clearing mMin. Otherwise, if it's not null, then we
		 * write the elements next to the min element around the min element to remove it, then arbitrarily reassign the min.
		 */
		...
		/*
		 * Next, clear the parent fields of all of the min element's children, since they're about to become roots. Because the
		 * elements are stored in a circular list, the traversal is a bit complex.
		 */
		...
		/*
		 * Next, we need to coalsce all of the roots so that there is only one tree of each degree. To track trees of each size, we
		 * allocate an ArrayList where the entry at position i is either null or the unique tree of degree i.
		 */
		List<Entry<T>> treeTable = new ArrayList<Entry<T>>();

		/*
		 * We need to traverse the entire list, but since we're going to be messing around with it we have to be careful not to break
		 * our traversal order mid-stream. One major challenge is how to detect whether we're visiting the same node twice. To do
		 * this, we'll spent a bit of overhead adding all of the nodes to a list, and then will visit each element of this list in
		 * order.
		 */
		List<Entry<T>> toVisit = new ArrayList<Entry<T>>();

		/*
		 * To add everything, we'll iterate across the elements until we find the first element twice. We check this by looping while
		 * the list is empty or while the current element isn't the first element of that list.
		 */
		...
		for (Entry<T> curr : toVisit) {
			...
			/*
			 * Update the global min based on this node. Note that we compare for <= instead of < here. That's because if we just did a
			 * reparent operation that merged two different trees of equal priority, we need to make sure that the min pointer points to
			 * the root-level one.
			 */
	...
	/**
	 * Decreases the key of the specified element to the new priority. If the new priority is greater than the old priority, this
	 * function throws an IllegalArgumentException. The new priority must be a finite double, so you cannot set the priority to be
	 * NaN, or +/- infinity. Doing so also throws an IllegalArgumentException.
	 *
	...
	public void delete(Entry<T> entry) {
		/*
		 * Use decreaseKey to drop the entry's key to -infinity. This will guarantee that the node is cut and set to the global
		 * minimum.
		 */
	...
	/**
	 * Utility function which, given two pointers into disjoint circularly- linked lists, merges the two lists together into one
	 * circularly-linked list in O(1) time. Because the lists may be empty, the return value is the only pointer that's guaranteed
	 * to be to an element of the resulting list.
	 *
	 * This function assumes that one and two are the minimum elements of the lists they are in, and returns a pointer to whichever
	 * is smaller. If this condition does not hold, the return value is some arbitrary pointer into the doubly-linked list.
	 *
	...
	public void decreaseKeyUnchecked(Entry<T> entry, double priority) {
		...
		/*
		 * If the node no longer has a higher priority than its parent, cut it. Note that this also means that if we try to run a
		 * delete operation that decreases the key to -infinity, it's guaranteed to cut the node from its parent.
		 */
		if (entry.mParent != null && entry.mPriority <= entry.mParent.mPriority)
			cutNode(entry);

		/*
		 * If our new value is the new min, mark it as such. Note that if we ended up decreasing the key in a way that ties the
		 * current minimum priority, this will change the min accordingly.
		 */
	...
	private void cutNode(Entry<T> entry) {
		...
		/*
		 * If the node is the one identified by its parent as its child, we need to rewrite that pointer to point to some arbitrary
		 * other child.
		 */
